...
if platform.node() in ('calipso','finlande','diamant','VirBox5HPHP'):
    ...
    gene_path = '/home/psakicki/THESE/DATA/1506_GEODESEA/WORKING/OPERA4//'
    gene_path = '/home/psakicki/THESE/DATA/1506_GEODESEA/WORKING/OPERA4/modded_with_tide'
    gene_path = '/home/psakicki/THESE/DATA/1506_GEODESEA/WORKING/OPERA4/modded_with_XYZnoised_1cm/'
    gene_path = '/home/psakicki/THESE/DATA/1506_GEODESEA/WORKING/OPERA4/modded_with_dephazage_100s/'
    gene_path = '/home/psakicki/THESE/DATA/1506_GEODESEA/WORKING/OPERA4/modded_with_dephazage_10s/'
    gene_path = '/home/psakicki/THESE/DATA/1506_GEODESEA/WORKING/OPERA4/modded_with_dephazage_-10s/'
    gene_path = '/home/psakicki/THESE/DATA/1506_GEODESEA/WORKING/OPERA4/modded_with_dephazage_0s/'
    ...
    gene_path = "/home/psakicki/GFZ_WORK/PROJECTS_OTHERS/1709_GNSSA_InSitu_EXPs/1808_reboot_GEODESEA_for_VB-HDR/REBOOT1808/classic"

elif "pgal" in platform.node():
    gene_path = "/dsk/ggsp_pf/PLAYGROUND/psakicki/1808_reboot_GEODESEA_for_VB-HDR/"
...
for iexp , exp in enumerate(exp_lis):
    ...
    if batch_mode:
        ...
        # Un run sur l'intégralité de la session de GEODESEA
        if 0:
        ...
        # Un run sur l'intégralité de la session de GEODESEA en decimant pour rapidité de calc

        if 1:
            bool_kw_lis = []
            
            booldic_var = genefun.boolean_dict(bool_kw_lis)
            #booldic_var['with_specific_timwin_bool'] = [21,22,23,24] 
        ...
        # multi jackknife mode, with cleaning and V as P
        if 0:
            bool_kw_lis = ['with_invert_jk' ]
            
            booldic_var = genefun.boolean_dict(bool_kw_lis)
            jk_rand_PROTOseed = 7891573
            N_jk = 18
            jk_PROTO_RandState = np.random.RandomState(jk_rand_PROTOseed)
            jk_rand_seed_lis = jk_PROTO_RandState.randint(10000,20000,N_jk)
            booldic_var['jk_rand_seed'] = jk_rand_seed_lis
            booldic_lis = geok.kwargs_for_jacobian({'with_ssp_bilin'      : 0,
                                                    'with_decimate_SSP'   : 1,               
                                                    'with_alternat_SSP'   : 0,
                                                    'with_barycenter'     : 0,
        ...
        if 0:
            ...
            bool_kw_lis = ['with_specific_timwin_bool' ]
            # Alors là, 2 cas :
            # with_specific_timwin_bool est un bouléen, cas simple, choix entre 2 périodes
            if 0:
                booldic_var = genefun.boolean_dict(bool_kw_lis)
            # with_specific_timwin_bool est pas un bouléen, cas plus complexe, 
            # pour des périodes plus sioux
            # On adopte un convention suivante 
            # 21 , 22 ... 1ere période de 2h de long , seconde période de 2h de long ...
            if 1:
                booldic_var = dict()
                booldic_var['with_specific_timwin_bool'] = [21,22,23,24] + list(np.arange(11,20))                       
                booldic_var['with_specific_timwin_bool'] = list(np.arange(11,20)) + [21,22,23,24]
                booldic_var['with_specific_timwin_bool'] = [99]
                booldic_var['with_specific_timwin_bool'] = list(np.arange(11,20))
                booldic_var['with_specific_timwin_bool'] = [11]
                booldic_var['with_specific_timwin_bool'] = [21,22,23,24] 
    ...
    if sigma_batch_mode:
        ...
        # Pour Geodesea
        sigmadic_var = {'sigma_pxp_apri'     : [0]    ,
                        'sigma_dZ_apri'      : [10**-12],
                        'sigma_BL_apri'      : [10**-12],
                        'sigma_defo_ASM'     : [10**-1 , 10**-20],
                        'sigma_defo_BL'      : [10**-3],
                        'sigma_defo_zmaster' : [10**-3]}
        # Pour Geodesea REBOOT 1704 on passe l'ASM à une val standard de 10**-6
        sigmadic_var = {'sigma_pxp_apri'     : [0]    ,
                        'sigma_dZ_apri'      : [10**-12],
                        'sigma_BL_apri'      : [10**-12],
                        'sigma_defo_ASM'     : [10**-6],
    ...
    for booldic , sigmadic in itertools.product(booldic_lis,sigmadic_lis):
        with_decimate_SSP  = 0
        jk_rand_seed       = 1452
        ...
        try:
            ...
            if not batch_mode:
                ...
                #ajout du 071216 pour avoir le barycentre
                with_barycenter   = 1
            ...
            if with_zmaster and not with_BL:
                print("ERR : with_monoZ and not with_BL, impossible combination, skipping ...")
                raise Exception
