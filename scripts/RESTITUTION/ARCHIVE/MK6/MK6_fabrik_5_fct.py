...
prm_x_grad        =  2.82625440384e-06   #1 * 10**-4
prm_y_grad        = 0       
prm_z_mixed       = 3900 #500
prm_bypass_ssf_rec = True

prm_with_pseudo_ssf = False
...
if imporved_noising:
    ...
    prm_sigma_zones = list(prm_sigma_zones)
    prm_zones_bound = list(prm_zones_bound)
...
if prm_traject == 'droite':
    ...
    prm_x0        = 10000
    prm_y0        = 10000 # start of the point 
    prm_xcenter   = 10000
    prm_ycenter   = 10000 # center of the circle
    prm_nb_obs    = 10
    prm_R         = 10
    prm_step_size = 10
...
def fabrik_fct(exp_prefix,midfix='',vari_dico=dict()):
    ...
    if prm_traject == 'droite':
        ...
        exp = '_'.join((exp_prefix  , "cross" , str(prm_nb_pass) + 'x' +  str(prm_nb_obs) ,
                        'x' + str(prm_xsize) , 'y' + str(prm_ysize) , 
                        "nois" + str(imporved_noising) + '-' + noise4exp , vitstr ,gradstr,
                         midfix , exp_suffix))
    elif prm_traject == 'derive':
    ...
    if prm_traject == 'droite':
        ...
        XYZreturned , E , circle = acls.fabriq_traject_derive(prm_x0,prm_y0,
                                             prm_xcenter,prm_ycenter,
        ...
        XYZreturned , E , _ = acls.fabriq_traject_cross(prm_xsize,prm_ysize,prm_xcenter,
                                                 prm_ycenter,prm_zcenter,prm_nb_pass,
                                                 prm_nb_obs,prm_angle,prm_vit,
                                                 epoch_init=prm_epoch_init,
                                                 plot=plot_in_fct,
    ...
    if prm_traject == 'derive':
        print(XYZ_emi_clean[0] , prm_x0 , prm_y0)
        ...
        for prm_ipxp,prm_pxp_coords in enumerate(PXP_lis): 
            ...
            if not prm_with_ssf:
                ...
                ssf_real = acls.make_SSF3D_from_SSP(SSPobjt,
                                               xmin=np.min(PXP_arr) - 1000,
                                               xmax=np.max(PXP_arr) + 1000,
                                               ymin=np.min(PXP_arr) - 1000,
                                               ymax=np.max(PXP_arr) + 1000)
            ...
            if not prm_with_ssf:
                ...
                N_t_hdwr         = np.random.RandomState(prm_K_t_hdwr_pxp)
                Tnse_hdwr = N_t_hdwr.randn(Tlen) * prm_sigma_t_hdwr
                Ttmp_stk  = [ Tnse_hdwr , np.array([0] * Tlen) ]
