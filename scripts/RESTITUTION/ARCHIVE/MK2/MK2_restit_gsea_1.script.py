...
bigdico  = acls.give_me_the_path(exp_path,exp ,[3,5,7])
...
if 1:
    while np.linalg.norm(PXPapri - PXPold) > 5* 10**-4 and iiter < iitermax:
...
print(Mcomm['pxp_coords'])
#
#
Obs , Mod = acls.vectorialize_ASM_multi([PXPnew],ObsASM_lis,Xbato,Z,C,nbprocs=nbproc)
V = Obs - Mod
#
##F.close()
