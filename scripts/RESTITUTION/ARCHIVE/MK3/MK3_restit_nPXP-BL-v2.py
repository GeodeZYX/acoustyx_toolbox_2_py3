...
for exp in exp_lis:
    exp_path = os.path.join(gene_path,exp + expdirsuffix)
...
#            while np.linalg.norm(PXPapri - PXPold) > 5 * 10**-5 and iiter <= iitermax:
            while np.linalg.norm(kriter) > 1 * 10**-4 and iiter <= iitermax:
            #while np.linalg.norm(kriter - kriterold) > 10**-4:
                ...
                # Partie Commune
                if with_BL:
                    _,_,P = geok.weight_mat([10**-5,10**-3],[len(ObsASM),len(ObsBL)],fuv)
                    A = np.vstack((JacobASM,JacobBL))
                    B = np.hstack((B_ASM,B_BL))   
                    #print 'WARN : exception a des fin de DEBUGGG !!!!'
                    #raise Exception
                else:
                    A = JacobASM
                    B = B_ASM
