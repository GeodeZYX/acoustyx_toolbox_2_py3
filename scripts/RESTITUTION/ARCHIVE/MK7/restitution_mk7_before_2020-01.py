...
#    else:
    ...
    try:
        ...
        #keep_ratio   = 0.5
        #keep_ratio   = 0.1
        #jk_rand_seed = 1452 Le jk_seed a Ã©tÃ© rammÃ©nÃ© avant l'interprÃ©tation du dico
...
#            raise Exception
            ...
            # les baselines sont injectÃ©es comme observables => mode contraint et non mode fixÃ©
            #Partie BL
            if with_bl:
                if with_barycenter:
            ...
            if with_zmaster:
                # Vieux bloc foireux (mÃ©lange des PXP observÃ©s/theoriques p/r au pxp master/ barycentre)
...
#                        lin[2] = -1  
            ...
            if not with_v_4_p_reinject or not "V_4_P_reinject" in list(globals().keys()): # V_4_P_reinject (2nd teste) dÃ©signe la liste et non le bouleen, pas touche
...
#                    A_stk.append(A)
            ...
            # la contrainte de barycentre est injectÃ©e suivant la methode fixÃ©e d'Helmert
            if with_barycenter:
            ...
            # ==== A partir de lÃ  on fat les conv memap / sparse
#                A = np.matrix(A)
#                B = np.matrix(B).T
#                P = np.matrix(P)
...
#                AtPB = np.dot(np.dot(A.T , P ) , B)  
        ...
        #    c,resid,rank,sigma = scipy.linalg.lstsq(A,B)
            ...
            debugrkmode = 0 # DiscontinuÃ©
            
            if with_zmaster and debugrkmode:
                # manip empirique de **** **** pour corriger la non nullitÃ©
                ...
                # Le debugrk mode est discontinuÃ© mais a grandement aidÃ© 
            ...
            if with_mono_z:
                dX = np.insert(dX,np.arange(2,len(dX)-2,2),dX[-1]) # manip sioux pour ajouter les dZ communs Ã  tous les PXP 
            if not with_barycenter:
                ...
                #OK c'Ã©tait un relicat d'une tentative de resolution du bug
                #chronique de non nullitÃ© des dZ (160609)
...
#                    Vnaif = np.concatenate((Vnaif,V_BL))
            ...
            if cleaning_coef != 0 and not ObsASMgoodbool is None: # pour quoi cleaning_coef != 0, et pourquoi ici ? chÃ©pa (160922)
        ...
        if plot_mode:
            ...
            for ipxp,pxp in enumerate(PXPnew_arr):
                ...
                if 1: # le bloc de test pour reproduire les ellipses
                    reload(acls)
                    # Les sigma sont normalisÃ© par le fuv (B) ou non (A)
                    ...
                    # C'est la fonction que l'on utilise jusqu'a prÃ©sent
                    ...
                    # c'est la fonction de Ghiliani, considÃ©riÃ© comme Ã©tant celle fiable
                    # phi => angle Ã  partir de X dans le sens trigo
                    # t   => angle Ã  partir de Y dans le sens horaire
                    ...
                    #gÃ©nÃ©ration de l'ellipse de Ghiliani, on inverse le signe de phi car
                    # ellipse_get_coords compte dans le sens horaire
                    xee , yee = geok.ellipse_get_coords(a*10000,b*10000,pxp[0],pxp[1],phi)
        
                    # une autre fonction basÃ©e sur Stang and Borre
