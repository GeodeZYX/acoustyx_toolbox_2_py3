...
gene_path = '/home/psakicki/THESE/CODES/CodePython/acoustyx_toolbox_2/working/BATC2_151020/'
...
expr_C_lambda     = sympy.lambdify(sym_var_lis,expr_C   , "numpy")  
...
#        for xbato , oasm , t in zip(Xbato , ObsASM , Tbato):
        for xbato_v , oasm  in zip(Xbato , ObsASM):
            ...
            #a titre informatif 
            # ici on compare les differentes valeurs obtenues suivant les diff traitements
            
            if 1:
                # fabrication de l'argument pour les lambdas
                Xbato = np.array([xbato_v[0],xbato_v[1],xbato_v[2]])
                Xpxp  = np.array([pxpapri[0],pxpapri[1],pxpapri[2]])
            
                args_4_lambdas = (xbato_v[0],xbato_v[1],xbato_v[2],
                               pxpapri[0],pxpapri[1],pxpapri[2],
                               0,c_v,oasm,0,0)
                               
                
                tau_line       = fctobs_lambda(*args_4_lambdas)
                dist           = np.linalg.norm(Xbato - Xpxp)
                ...
                angl     = expr_angl_lambda(*args_4_lambdas)
                Cline    = expr_C_lambda(*args_4_lambdas)
                Ctheorik = dist / tau_klassic
                
                ifo_tau_line.append(tau_line)
                ifo_tau_klassic.append(tau_klassic)
                ifo_tau_obs.append(oasm)
                
                ifo_angl.append(angl)
                ifo_Cline.append(Cline)
                ifo_Ctheorik.append(Ctheorik)
                ifo_dist.append(dist)
                

                               
            if 0:    
                # Il s'agit d'un test totalement independant
...
ifo_tau_line    = np.array(ifo_tau_line)
ifo_tau_klassic = np.array(ifo_tau_klassic)
ifo_tau_obs     = np.array(ifo_tau_obs)
ifo_angl        = np.array(ifo_angl)
ifo_Cline       = np.array(ifo_Cline)
ifo_Ctheorik    = np.array(ifo_Ctheorik)
ifo_dist        = np.array(ifo_dist)

np.nanmax(ifo_tau_klassic - ifo_tau_obs)
np.nanmax(ifo_tau_line - ifo_tau_obs)

dtau = ifo_tau_klassic - ifo_tau_line

plt.close('all')
plt.figure()
plt.plot(dtau)
plt.figure()
plt.plot(ifo_Ctheorik - ifo_Cline)
plt.figure()
plt.plot(ifo_Cline,'+b')
plt.plot(ifo_Ctheorik,'+r')
