...
if platform.node() == 'calipso':
    ...
    gene_path = '/home/psakicki/CODES/acoustyx_toolbox_2b/working/'
    alternat_SSP_path='/home/psakicki/CODES/acoustyx_toolbox_2b/exemple/SSP/SSP_NOAA_dep5781_20050827000000'
...
for exp in exp_lis:
    ...
    for booldic in booldic_lis:
        try:
            ...
            # FONCTION D'OBS BASELINE
            sym_var_BL_tup = sympy.symbols('xpxp1 ypxp1 zpxp1 xpxp2 ypxp2 zpxp2')
            xpxp1 , ypxp1 , zpxp1 , xpxp2 , ypxp2 , zpxp2 = sym_var_BL_tup
            sym_var_BL_lis = list(sym_var_BL_tup)
            expr_BL    = sympy.sqrt((xpxp1 - xpxp2) ** 2 + (ypxp1 - ypxp2) ** 2 + (zpxp1 - zpxp2) ** 2)  
            bl_diff_list        = []
            bl_diff_lambda_list = []
            for sym_var in sym_var_BL_lis:                
                bl_diff = sympy.diff(expr_BL,sym_var)
                bl_diff_lambda = sympy.lambdify(sym_var_BL_lis,bl_diff, "numpy") 
                bl_diff_list.append(bl_diff)
                bl_diff_lambda_list.append(bl_diff_lambda)
...
#            while np.linalg.norm(PXPapri - PXPold) > 5 * 10**-5 and iiter <= iitermax:
            ...
            #        for xbato , oasm , t in zip(Xbato , ObsASM , Tbato):
                ...
                if with_BL:
                    ...
                    for ii in range(BL.shape[1]):
                        for jj in range(BL.shape[1]):
                            if jj <= ii:
                                continue
                            else:
                                i = ii
                                j = jj
                            print(PXPapri_lis[i] , PXPapri_lis[j], end=' ') 
                            
                            argtup_bl = tuple(PXPapri_lis[i]) + tuple(PXPapri_lis[j])                            
                            xpxp1diff = bl_diff_lambda_list[0](*argtup_bl)
                            ypxp1diff = bl_diff_lambda_list[1](*argtup_bl)
                            zpxp1diff = bl_diff_lambda_list[2](*argtup_bl)
                            pxp1diff  = np.array([xpxp1diff , ypxp1diff , zpxp1diff])
                            xpxp2diff = bl_diff_lambda_list[3](*argtup_bl)
                            ypxp2diff = bl_diff_lambda_list[4](*argtup_bl)
                            zpxp2diff = bl_diff_lambda_list[5](*argtup_bl)
                            pxp2diff  = np.array([xpxp2diff , ypxp2diff , zpxp2diff])
                            
                            pxp1diffB , pxp2diffB = acls.diff_dist(PXPapri_lis[i] , PXPapri_lis[j])

                            if not with_c0_estim:
                                line = np.zeros(nPXP * 3)
                            else:
                                if with_monoc0:
                                    line = np.zeros(nPXP * 3 + 1)
                                    nnn = 3
                                else:
                                    line = np.zeros(nPXP * 3 + nPXP)
                                    nnn = 4
                                    
                            line[i*nnn:i*nnn+3] = pxp1diff
                            line[j*nnn:j*nnn+3] = pxp2diff
                            
                            Abl_line_stk.append(line)
                        
                    Abl = np.vstack(Abl_line_stk)
                    
                    Abl2  =  np.column_stack((acls.jacob_BL(PXPapri_lis),np.zeros(len(Bbl))))
                    
                    Abl_stk.append(Abl)
                    Abl2_stk.append(Abl2)
                ...
                if with_zmaster:
                    Bz = []
                    Az_stk = []
                    for ipxp in range(nPXP):
                        
                        pxpapri = PXPapri_lis[ipxp]
                        pxpmasterapri = PXPapri_lis[i_pxp_master]
                        
                        if ipxp == i_pxp_master:
                            continue
                        
                        Bz.append( PXPdZ_lis[ipxp] - (pxpapri[-1] - pxpmasterapri[-1]))
                ...
                Psprs = sprs.csc_matrix(sprs.diags(P,0))
...
#                AtPB = np.dot(np.dot(A.T , P ) , B)  
                ...
                del AtPBsprs , Asprs , Bsprs , Psprs
                ...
                V = np.squeeze(B) - np.squeeze(A).dot(np.squeeze(dX))
