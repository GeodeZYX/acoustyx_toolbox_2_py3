...
if platform.node() == 'calipso':
    ...
    expdirsuffix = 'dZADEL'    
    exp  = "ADOCdZ1_1500_R100_nois1-1e-06_vit1__"
    expdirsuffix = ''
    exp = 'ADOCdZ1_3x100_x2000_y2000_nois1-1e-06_vit1__'
    exp = 'ADOCdZ1_1500_R1500_nois1-1e-06_vit1__'
    exp = 'ADOCdZ1_3x100_x2000_y2000_nois1-1e-06_vit1__'
...
for exp in exp_lis:
    ...
    for booldic in booldic_lis:
        ...
        try:
            ...
            sigma_pxp_apri = 1
            kmeta_pxp_apri = 127 # le K normal c'est l'ID du PXP, le meta c'est pour tous
...
#                        lin[2] = -1  
                ...
                if not with_V_4_P_reinject or not "V_4_P_reinject" in list(globals().keys()):
                    ...
                    sigma_4_P = sigma_4_P + [V_4_P_reinject]
                    len_4_P.append(len(V_4_P_reinject))
...
#                AtPB = np.dot(np.dot(A.T , P ) , B)  
                ...
                if with_barycenter:
                    AtPBbis     = np.vstack((AtPB,np.matrix(np.zeros(Gsiz)).T)) ##MODIF
                    #AtPBbis[-1] = -63.524075746381186 #acls.barycenter_calc(PXPtrue_arr[:,-1])
...
#                    Vnaif = np.concatenate((Vnaif,V_BL))
            ...
            plt.axis('equal')
            plt.savefig(os.path.join(exp_path , exp + "_geo.png"))
