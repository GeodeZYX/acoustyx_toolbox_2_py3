...
if platform.node() in ('calipso','finlande','diamant'):
        ...
        try:
            ...
            if with_time_window:
                ...
                for iiiddd , (ttt,obsasm) in enumerate(zip(TTT_lis,ObsASM_lis_orig)):
                    ...
                    TTT_lis_4_resid.append(np.column_stack(([iiiddd + 1] * len(epocoutobsasm) , epocoutobsasm)))
                    
            else: # not with time win
                TTT_lis_4_resid = []
                for iiiddd in range(TTT_lis):
                    TTT_lis_4_resid.append(np.column_stack(([iiiddd + 1] * len(TTT_lis[iiiddd]),TTT_lis[iiidddd])))
...
#                    Vnaif = np.concatenate((Vnaif,V_BL))
            ...
            if plot_mode:
                ...
                aaa = 0
                bbb = 0
                V_per_ObsASM_4_plot = gf.sublistsIt(V_4_P_reinject,lenObsASM,True)
                for Arrbool , IdnTtt  in zip(ObsASMgoodbool,TTT_lis_4_resid):
                    aaa = aaa + len(IdnTtt[Arrbool])
                    bbb = bbb + np.sum(Arrbool)
                
                print(len(V_4_P_reinject))
            ...
            if plot_mode:
                plt.clf()
                if with_FB_mode:
                    plt.plot(np.vstack(Xbato_f)[:,0], np.vstack(Xbato_f)[:,1], 'gx')
                ...
                for ipxp,pxp in enumerate(PXPnew_arr):
                    if with_barycenter:
                        ...
                        kkk = 0
                    if 0: # le bloc de test pour reproduire les ellipses
                        reload(acls)
