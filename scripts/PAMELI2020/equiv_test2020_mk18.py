...
mk17 : cumulative window
mk18 : window with unik circles 
"""
#### Import star style
...
elif param_mode == 7: #range good smaller

    Tab_p_twtt     = [2e-05]*6
    Tab_p_dir_vect = [0,0.01,0.001]*2
    
    Tab_estim_c                = [0,0,0,1,1,1]
    Tab_with_direction_vectors = [0,1,1,0,1,1]
...
Range_iTab_param = [4,7]                        ### weight range mono => worst one
...
Range_iTab_path  = np.arange(0,9)

Range_iTab_win   = np.arange(0,5)
Range_iTab_win   = [-1] 
Range_iTab_win   = np.arange(0,34) ### unik
Range_iTab_win   = np.arange(0,5) ### chunk
Range_iTab_win   = np.arange(0,10) ### slice
...
for iTab_path,iTab_param,iTab_win,iTab_bea  in itertools.product(Range_iTab_path,
    ...
    try:
        ...
        path_obs = path_computer + "/02_PREPROCESSING/07_d/"
        path_obs = path_computer + "/02_PREPROCESSING/03_o/"
       ...
       ########## REAL TIME IXBLUE - Up from iXBlue
        ...
        P_obs_stk.append(p_obs)
        p_obs = path_obs + "PAM_BST_v232_m2507-1245_m4_d06_bea3_RTiXBlue/PAM_BST_v232_m2507-1245_m4_d06_bea3_RTiXBlue.csv"
        P_obs_stk.append(p_obs)
    ...
    #   P_obs_stk.append(p_obs)
        ...
        if with_period_select or iTab_win != -1:
            ...
            Tab_win_len_twtt = np.arange(.1,1.1,.1)
            Tab_win_srt_twtt = [0.] * len(Tab_win_len_twtt)  
            

            ######################################################
            ########### MONO CIRCLES #############################
            DFtmp = pd.read_csv(p_obs,index_col=0)
            bea = np.sort(DFtmp['ID_BEA'].unique())[0]
            spikedic = utils.pickle_loader("/wrk/sakic_xchg/2009_PAMELi_GNSSA/02_PREPROCESSING/spike_dict.pik")
            Tab_win_twtt_raw = spikedic[bea] 
            ...
            if mode == "unik": 
                Tab_win_twtt = Tab_win_twtt_raw       
            elif mode == "slice":
                Tab_win_twtt = np.vstack([(e[0,0],e[:,1].sum(),e[-1,-1]) for e in utils.sliceIt(Tab_win_twtt_raw,4)])
            elif mode == "chunk":
                Tab_win_twtt = np.vstack([(e[0,0],e[:,1].sum(),e[-1,-1]) for e in utils.chunkIt(Tab_win_twtt_raw,5)])            

            Tab_win_srt_twtt = Tab_win_twtt[:,0] 
            Tab_win_len_twtt = Tab_win_twtt[:,1]

            if iTab_win >= len(Tab_win_len_twtt):
                print("WARN: we skip the experience because no more spike window")
                F.stop()
                continue
            ...
            print("INFO:win str/len",win_srt_twtt,win_len_twtt)
        ...
        ######################### DIRECTION VECTORS #########################
            ...
            DictIteraStore_iter['ping_valid']        = np.sum(DFbea['VALID']) 
            DictIteraStore_iter['ping_total']        = len(DFbea) 


            if with_period_select: 
                DictIteraStore_iter['win_str'] = win_srt_twtt
                DictIteraStore_iter['win_len'] = win_len_twtt
                DictIteraStore_iter['idx_str'] = idx_srt
                DictIteraStore_iter['idx_len'] = idx_len
