...
path_bl  = "/home/psakicki/GFZ_WORK/PROJECTS_OTHERS/2001_PAMELi_GNSSA/02_PREPROCESSING/01_/PAMELI_BREST_vJupyter_2_4transducers/PAMELI_BREST_vJupyter_2_4transducers_BASELINE.csv"
...
estim_c                = Tab_estim_c[i_Tab_param]               
with_direction_vectors = Tab_with_direction_vectors[i_Tab_param] 
p_twtt                 = Tab_p_twtt[i_Tab_param]                
p_dir_vect             = Tab_p_dir_vect[i_Tab_param]

with_period_select  = 1
...
if with_period_select:
    ...
    win_srt_twtt=Tab_win_srt_twtt[i_Tab_win]
    win_len_twtt=Tab_win_len_twtt[i_Tab_win]
...
######################### DIRECTION VECTORS #########################
    ...
    fuv = V.dot(P).dot(V) / (A.shape[0] - A.shape[1])
    Varcovar = fuv * Ninv
    ...
    DictIteraStore_iter['globals']           = globals().copy()
    ...
    ###### Last Saves
    DictIteraStore_iter['DFdirvect_big'] = copy.deepcopy(DFdirvect_big)
    DictIteraStore_iter['globals_end'] = globals().copy()
...
mu = np.mean(B_1stiter_valid)
sigma = np.std(B_1stiter_valid)
x = np.linspace(mu - 3*sigma, mu + 3*sigma, 100)
axhist.hist(B_1stiter_valid,nbin,density=True,label="1st iter")
axhist.plot(x, scipy.stats.norm.pdf(x, mu, sigma))

mu = np.mean(B_lastiter)
sigma = np.std(B_lastiter)
x = np.linspace(mu - 3*sigma, mu + 3*sigma, 100)
axhist.hist(B_lastiter,nbin,density=True,label="last iter")
axhist.plot(x, scipy.stats.norm.pdf(x, mu, sigma))
