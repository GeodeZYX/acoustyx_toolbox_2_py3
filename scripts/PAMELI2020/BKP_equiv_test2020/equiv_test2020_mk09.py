...
for i_itera in range(n_itera):
    ...
    if with_direction_vectors:
        DFbig['vN_mod'] = np.zeros(len(DFbig['TWTT']))
        DFbig['vE_mod'] = np.zeros(len(DFbig['TWTT']))
        DFbig['vD_mod'] = np.zeros(len(DFbig['TWTT']))

        DFbig['A_vN_x'] = np.zeros(len(DFbig['TWTT']))
        DFbig['A_vN_y'] = np.zeros(len(DFbig['TWTT']))
        DFbig['A_vN_z'] = np.zeros(len(DFbig['TWTT']))

        DFbig['A_vE_x'] = np.zeros(len(DFbig['TWTT']))
        DFbig['A_vE_y'] = np.zeros(len(DFbig['TWTT']))
        DFbig['A_vE_z'] = np.zeros(len(DFbig['TWTT']))
        
        DFbig['A_vD_x'] = np.zeros(len(DFbig['TWTT']))
        DFbig['A_vD_y'] = np.zeros(len(DFbig['TWTT']))
        DFbig['A_vD_z'] = np.zeros(len(DFbig['TWTT']))

        DFbig['B_vN'] = np.zeros(len(DFbig['TWTT']))
        DFbig['B_vE'] = np.zeros(len(DFbig['TWTT']))
        DFbig['B_vD'] = np.zeros(len(DFbig['TWTT']))    
    ...
    for i_idbea, idbea in enumerate(ID_bea_list):
        ...
        for i_rowbea , rowbea in DFbea.iterrows():
            ...
            if rowbea.date_emi != prev_rowbea_date_emi:
                prev_rowbea_date_emi = rowbea.date_emi
            ...
            if with_direction_vectors:
                ...
                xrec_AHD = np.array(rowbea[cols_posi_rec_AHD].values,dtype=float)
                
                dir_vect_obs = rowbea[["vN_nrm","vE_nrm","vD_nrm"]].values.astype(np.float64)
            ...
            if not fast_mode:
                ...
                if with_direction_vectors and add_dir_vect_to_AnB:
                    for idirvect in range(3):
                        Bfast_stk.append(B_dir_vect[idirvect])
                        Bfast_dirvect_only_stk.append(B_dir_vect[idirvect])
                    Irow_stk.append(i_rowbea)
            ...
            if with_direction_vectors and add_dir_vect_to_AnB:
                #from pygoat.seafloorpos import ixblue_process_fcts as ibpf
                #DirVectDIFF1 = ibpf.fct_obs_dir_vect_deriv_symbo(*args_for_partial_dev_dir_vect) 
                #boll_equiv = np.all(np.isclose(np.array(DirVectDIFF1,np.float64) , np.array(DirVectDIFF2)))
                #print(boll_equiv)
                
                DirVectDIFF2 = ibpf.fct_obs_dir_vect_deriv_ana(*args_for_partial_dev_dir_vect)
                
                DirVectDIFF  = DirVectDIFF2
                                
                if not fast_mode:
                    rowbea[['A_vN_x','A_vN_y','A_vN_z']] = DirVectDIFF[0:3]
                    rowbea[['A_vE_x','A_vE_y','A_vE_z']] = DirVectDIFF[3:6]
                    rowbea[['A_vD_x','A_vD_y','A_vD_z']] = DirVectDIFF[6:9]
                    
                else:
                    Afast_row = list(DirVectDIFF[0:3]) + [0]
                    Afast_stk.append(Afast_row)
                    Afast_row = list(DirVectDIFF[3:6]) + [0]
                    Afast_stk.append(Afast_row)
                    Afast_row = list(DirVectDIFF[6:9]) + [0]
                    Afast_stk.append(Afast_row)
    ...
    ############### DEBUG
    Bdir_vect = np.array(Bfast_dirvect_only_stk)
    ...
    if with_direction_vectors:
        ...
        K , Q , P = stats.weight_mat([p_twtt,p_angle]*size,[1,3]*size)
...
if True:
    ...
    #                      'E_' +tdcstr+ '_rec',
    #                      'D_' +tdcstr+ '_rec']
    #     axte.plot(DFtsc["date_rec"],DFtsc[cols_posi_rec[1]],"x")
    #     axtn.plot(DFtsc["date_rec"],DFtsc[cols_posi_rec[0]],"x")
...
for idbea in DFbig.ID_BEA.unique():    
    ...
    axhist.hist(DFbeaplt_valid["B_TWTT"],50,density=0)
