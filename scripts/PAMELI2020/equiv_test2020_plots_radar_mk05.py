...
col_static_long_short_name_dict["BaAM"] = "Array-centered (AM)"
col_static_long_short_name_dict["BaPM"] = "Array-centered (PM)"
col_static_long_short_name_dict["ABOV"] = "Above beacon"    
...
def core_legend(figax_in,DFbea_in):
        ...
        if mode in ("simple_range","win_range"):
            ...
            for iparam , (cest,wdv) in DFbea[["C_esti","W_DV"]].drop_duplicates().iterrows():                    
                ...
                lab = "C estim. " + str(cest) + ", " + str(wdvstr) 
...
for idbea in range(1,4):
    ...
    if mode == "win_range":
        ...
        #                       center     dE dN
        extra_size_dict[1] = ((24.5,-7.4),(.6,.6))
        extra_size_dict[2] = ((-26.2,-7.2),(.6,.6))
        extra_size_dict[3] = ((2.1,22.5),(.6,.6))

    elif mode == "simple_range":
        DFcore_in = DFbea
        mode_core_in = mode
        figax = core_plot(DFcore_in,mode_in=mode_core_in,refpoint=refpt)
        core_ref_pt(figax,DFcore_in)
    ...
    if len(extra_size_dict) > 0:
        ((xcentr,ycentr),(xstp,ystp)) = extra_size_dict[idbea]
        figax[1].set_xlim((xcentr-xstp,xcentr+xstp))
        figax[1].set_ylim((ycentr-ystp,ycentr+ystp))
        outtype = (".png",".pdf",".figpik")
        figoutpath = utils.figure_saver(figax[0],
                           os.path.dirname(pikpath),
                           outname + "_zoom",
