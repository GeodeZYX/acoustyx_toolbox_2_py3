...
if 1:
    ...
    if 0:
        gf.pickle_saver(SensGrp,path,'sens_grp',timestamp=True)
    if 1:
        SensGrp = gf.pickle_loader("/home/psakicki/THESE/bd_CTD/NEW_DATA_2015/MOVE/MOVE_reform_1507/SIMPLE/20160408_113539_sens_grp.pik")
    if 1:
        Ltemp = [ (sens.id , np.column_stack(sens.get_TZC())) for sens in SensGrp.grp ]
        dico = dict()
        for e in Ltemp:
            dico[e[0]] = e[1]
        gf.pickle_saver(dico,path,'sens_grp_minidico',timestamp=True)
...
# C'est comme par PART1 mais pour tous les senseurs

reload(mcls)

min_extrem_sv = np.min([sensor.min_sv for sensor in SensGrp.grp])
max_extrem_sv = np.max([sensor.max_sv for sensor in SensGrp.grp])
    ...
    for period_size in period_size_lis:
        ...
        for sensor in SensGrp.grp:
            ...
            for winsens in windoweds_sensor_lis[:]:
                ...
                mcls.plot_TC_frontend(winsens,1,path=pathplot2,
                                      minmax=[sensor.min_sv,sensor.max_sv],
                                      minmax_general=[min_extrem_sv,max_extrem_sv])

# PART 2.5 : PLOTs TEMPORELS SUR TOUS LES SENSEURS (OPERATIONNEL)
# C'est comme par PART1 mais pour tous les senseurs
# c'est comme pour la PART2 mais en vielle version  
    ...
    for period_size in period_size_lis:
        ...
        for sensor in SensGrp.grp:
            ...
            for winsens in windoweds_sensor_lis[:]:
                pathplot2 = os.path.join(pathplot,str(period_size),str(winsens.id))
                gf.create_dir(pathplot2)
                mcls.plot_TC_frontend(winsens,1,path=pathplot2,minmax=[sensor.min_sv,
                                                                       sensor.max_sv],
                                                                       plotmod=1)
